...
map, and `parser_sha256`. It records `api_version: 2`, the three decoder v2 `exports` plus
`AacEncoder_GetTiming` and `Decoder_GetInputFormat`,
`execution_host_rid`, `drain_pass: true`, `error_pass: true`, `encoder_timing_pass: true`, `input_format_pass: true`, and nonempty identity
...
drain, decoded input-format, failure and encoder-timing consumers before issuing a receipt. Provenance signing and

...
for case_name in single-complete multipart-complete single-construction-failure multipart-construction-failure single-flush-failure multipart-flush-failure single-write-failure multipart-write-failure; do
  dotnet "$HARNESS" "$PROBE" "$INPUT" "$case_name" > "$LOGS/$case_name.log" 2>&1
  cat "$LOGS/$case_name.log"
done
printf '%s\n' 'PASS 8/8 AAC pipeline native lifetime cases'

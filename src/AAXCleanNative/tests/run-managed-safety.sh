...
DYLD_INSERT_LIBRARIES="$(/usr/bin/xcrun clang -print-file-name=libclang_rt.asan_osx_dynamic.dylib)"
...
dotnet "$ARTIFACTS/managed/bin/NativeHandleSafety/debug/NativeHandleSafety.dll" \
  "$ARTIFACTS/asan/tests/libnative-missing-timing.dylib" missing-timing > "$ARTIFACTS/managed-missing-timing.log" 2>&1
cat "$ARTIFACTS/managed-missing-timing.log"

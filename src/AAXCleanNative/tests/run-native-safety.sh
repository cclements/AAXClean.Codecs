...
"$CC" "${FLAGS[@]}" -DBUILD_MANAGED_PROBE -DOMIT_ENCODER_TIMING -dynamiclib \
  "$NATIVE_SOURCE/tests/native-safety.c" "${LIBS[@]}" -o "$BUILD/tests/libnative-missing-timing.dylib"
for case_name in abi extradata-boundary packet-boundary malformed-packets packet-errors failed-opens encoder-timing; do
  "$BUILD/tests/native-safety" "$case_name" > "$BUILD/tests/$case_name.log" 2>&1
  cat "$BUILD/tests/$case_name.log"
done
/usr/bin/shasum -a 256 "$BUILD/tests/native-safety" "$BUILD/tests/libnative-safety.dylib" "$BUILD/tests/libnative-missing-timing.dylib" \
